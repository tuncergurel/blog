...
def Currency(request): #Currency'nin Mssql'den alındığı alan
    cur = sqlConnect()
    query = """select  * from currency where currencynumber between 1 and 12 and exchangedate = (cast(GETDATE()-1 as date)) """
...
def Cryptos(request): #Currency'nin Mssql'den alındığı alan
    cur = sqlConnect()
    query2 = """select top 1000 * from cryptocurrency order by last_updated desc, coin_no asc"""
    data = cur.execute(query2).fetchall()
...
def cryptomoves(request):
    ...
    cur = sqlConnect()
    query = """select top 600 * from cryptocurrency order by last_updated desc, coin_no asc"""
    ...
    plt.title("{}'s Last 2 Hours".format((name[0].upper())))
    plt.xlabel('Time (UTC)')
    ...
    query2 = """ select top 20 coin_symbol from cryptocurrency order by last_updated desc """

    get_coins = cur.execute(query2).fetchall()
    coins = []
    for i in get_coins:
        coins.append(i[0])
